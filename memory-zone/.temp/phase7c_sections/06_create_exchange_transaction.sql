-- =====================================================================
-- 6. Äá»•i hÃ ng â€” Stock Entry (tráº£ + bÃ¡n)
-- =====================================================================
...
BEGIN
  ...
  IF NOT v_has_return AND NOT v_has_exchange THEN
    RAISE EXCEPTION 'Phiáº¿u trá»‘ng: pháº£i cÃ³ Ã­t nháº¥t 1 sáº£n pháº©m tráº£ hoáº·c 1 sáº£n pháº©m Ä‘á»•i'
      USING ERRCODE = 'P0001';
  END IF;
  ...
  -- PHáº¦N A â€” TRáº¢ HÃ€NG
  -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  IF v_has_return THEN
    IF p_return_id IS NULL OR p_return_id = '' THEN
      RAISE EXCEPTION 'return_id is required khi cÃ³ hÃ ng tráº£';
    END IF;
    IF p_original_order_id IS NULL OR p_original_order_id = '' THEN
      RAISE EXCEPTION 'original_order_id is required khi cÃ³ hÃ ng tráº£';
    END IF;
    IF p_total_refund_amount < 0 THEN
      RAISE EXCEPTION 'Tá»•ng tiá»n hoÃ n khÃ´ng thá»ƒ Ã¢m';
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'ÄÆ¡n hÃ ng gá»‘c % khÃ´ng tá»“n táº¡i', p_original_order_id;
    END IF;
    ...
    IF v_new_total_returned > COALESCE(v_order.total_amount, 0) + 0.01 THEN
      RAISE EXCEPTION 'Tá»•ng tiá»n tráº£ (%) vÆ°á»£t quÃ¡ tá»•ng tiá»n Ä‘Æ¡n hÃ ng (%)',
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_return_items) LOOP
      ...
      IF v_qty <= 0 THEN
        RAISE EXCEPTION 'Sá»‘ lÆ°á»£ng tráº£ cá»§a "%" pháº£i > 0', v_item->>'productName';
      END IF;
      ...
      IF v_ordered_qty = 0 THEN
        RAISE EXCEPTION 'Sáº£n pháº©m "%" khÃ´ng cÃ³ trong Ä‘Æ¡n hÃ ng gá»‘c',
          v_item->>'productName'
          USING ERRCODE = 'P0001';
      END IF;
      ...
      IF v_already_returned + v_qty > v_ordered_qty + 0.001 THEN
        RAISE EXCEPTION 'Tráº£ vÆ°á»£t sá»‘ Ä‘Ã£ bÃ¡n cho "%" (Ä‘Ã£ bÃ¡n %, Ä‘Ã£ tráº£ %, Ä‘ang tráº£ thÃªm %)',
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_return_items) LOOP
      ...
      -- Phase 7c: GHI BÃšT TOÃN Sá»” CÃI KHO â€” TRáº¢
  ...
  -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  -- PHáº¦N B â€” ÄÆ N Äá»”I Má»šI
  -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  IF v_has_exchange THEN
    IF p_exchange_order_id IS NULL OR p_exchange_order_id = '' THEN
      RAISE EXCEPTION 'exchange_order_id is required khi cÃ³ hÃ ng Ä‘á»•i';
    END IF;
    IF p_exchange_total < 0 THEN
      RAISE EXCEPTION 'exchange_total khÃ´ng thá»ƒ Ã¢m';
    END IF;
    ...
    INSERT INTO orders (
      ...
      '[]'::jsonb,
      CASE
        WHEN v_has_return THEN 'ÄÆ¡n Ä‘á»•i tá»« phiáº¿u tráº£ ' || COALESCE(p_return_id, '?')
          || CASE WHEN p_is_delivery THEN ' (Giao hÃ ng)' ELSE '' END
        ELSE
          CASE WHEN p_is_delivery THEN 'ÄÆ¡n bÃ¡n (Giao hÃ ng)' ELSE 'ÄÆ¡n bÃ¡n Ä‘á»•i' END
      END
    );
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_exchange_items) LOOP
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Sáº£n pháº©m % khÃ´ng tá»“n táº¡i trong kho', v_product_id
          USING ERRCODE = 'P0001';
      END IF;
      ...
      IF v_product.has_lots THEN
          ...
          BEGIN
            ...
            IF v_remaining_qty > 0 THEN
              IF NOT p_allow_negative THEN
                RAISE EXCEPTION 'Tá»“n kho lÃ´ cá»§a "%" khÃ´ng Ä‘á»§ (cáº§n %, thiáº¿u %)', v_product.name, v_qty, v_remaining_qty;
              END IF;
        ...
        ELSE
          ...
          BEGIN
            ...
            IF NOT FOUND THEN
              RAISE EXCEPTION 'LÃ´ chá»‰ Ä‘á»‹nh % khÃ´ng tá»“n táº¡i cho sáº£n pháº©m "%"', v_ex_lot_id, v_product.name;
            END IF;

            IF NOT p_allow_negative AND v_lot_qty < v_qty THEN
              RAISE EXCEPTION 'LÃ´ chá»‰ Ä‘á»‹nh "%" khÃ´ng Ä‘á»§ tá»“n (cáº§n %, cÃ²n %)', v_ex_lot_id, v_qty, v_lot_qty;
            END IF;
      ...
      ELSE
        IF NOT p_allow_negative AND COALESCE(v_product.quantity, 0) < v_qty THEN
          RAISE EXCEPTION 'Tá»“n kho khÃ´ng Ä‘á»§ cho "%" (cáº§n %, cÃ²n %)', v_product.name, v_qty, v_product.quantity
            USING ERRCODE = 'P0001';
        END IF;
      ...
      -- Phase 7c: GHI BÃšT TOÃN Sá»” CÃI KHO â€” BÃN
      v_qty_after := get_product_stock_balance(v_product_id, NULL);
