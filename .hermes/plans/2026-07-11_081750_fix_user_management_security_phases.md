...
**Source:** 
**Status:** ✅ **Đã fix và đã push production**
...
**Source:** 
**Status:** ✅ **Đã fix và đã deploy production**
...
| Task | Fix | Migration / Artifact | Production Status |
|------|-----|----------------------|-------------------|
| [7.1] | `remove_system_admin` RPC `SECURITY INVOKER` | `20260712000004_fix_remove_system_admin_security_definer.sql` | ✅ Pushed |
| [7.2] | Guardrail trigger filter `status`/`is_active` | `20260712000005_fix_guardrail_trigger_status_active_filter.sql` | ✅ Pushed |
| [7.3] | `toggleMemberActive` guardrail protection | `20260712000005_fix_guardrail_trigger_status_active_filter.sql` | ✅ Pushed |
| [7.4] | `reset-password` Edge Function rate limiting | `supabase/functions/reset-password/index.ts` | ✅ Deployed |
| [7.5] | `is_system_admin()` service-role bypass | `20260712000011_fix_is_system_admin_service_role.sql` | ✅ Pushed |

**Notes:**
- `20260712000011` was applied to production; `20260711021026` (same fix, earlier timestamp) already existed in remote migration history.
- Edge Function `reset-password` was redeployed; CLI reported "No change found" because the remote version already matches local.
- `20260712000013_add_viewer_role.sql` (Phase 6) was pushed to production after Phase 5.

## Phase 6: ✅ Complete - Architecture & Migration Rollup
...
**Status:** ✅ **Đã xong và đã deploy lên production project `rsialbfjswnrkzcxarnj`**

**Date:** 2026-07-11

### Migrations (all 12 pushed to production)

| Migration | Description | Production Status |
|-----------|-------------|-------------------|
| `20260712000001_fix_remove_tenant_member_rpc.sql` | `remove_tenant_member` RPC with SECURITY DEFINER + auth guards | ✅ Pushed |
| `20260712000002_fix_update_tenant_member_role_rpc.sql` | `update_tenant_member_role` RPC with role validation | ✅ Pushed |
| `20260712000003_fix_toggle_tenant_member_active_rpc.sql` | `toggle_tenant_member_active` RPC with guardrails | ✅ Pushed |
| `20260712000004_fix_remove_system_admin_security_definer.sql` | `remove_system_admin` SECURITY DEFINER fix | ✅ Pushed |
| `20260712000005_fix_guardrail_trigger_status_active_filter.sql` | Guardrail trigger filter `status`/`is_active` | ✅ Pushed |
| `20260712000006_add_soft_delete_columns.sql` | Add soft-delete columns | ✅ Pushed |
| `20260712000007_add_rls_policies_tenant_memberships.sql` | RLS policies for `tenant_memberships` | ✅ Pushed |
| `20260712000008_add_audit_log_triggers.sql` | Audit log triggers | ✅ Pushed |
| `20260712000009_add_advisory_lock_function.sql` | Advisory lock function | ✅ Pushed |
| `20260712000010_fix_get_tenant_usage_summary_tenant_admin.sql` | `get_tenant_usage_summary` tenant admin fix | ✅ Pushed |
| `20260712000011_fix_is_system_admin_service_role.sql` | `is_system_admin()` service-role bypass | ✅ Pushed |
| `20260712000012_add_system_admin_for_edge.sql` | `add_system_admin_for_edge` RPC for Edge Functions | ✅ Pushed |

### Edge Functions (redeployed to production)

| Edge Function | Production Status |
|---------------|-------------------|
| `create-system-admin` | ✅ Redeployed |
| `create-tenant` | ✅ Redeployed |
| `invite-member` | ✅ Redeployed |
| `reset-password` | ✅ Redeployed |
| `delete-tenant` | ✅ Redeployed |
| `delete-user` | ✅ Redeployed |

### Notes
- `20260711020807_add_viewer_role.sql` and `20260711021026_fix_is_system_admin_service_role.sql` were already present in production migration history; duplicate local copies were removed.
- `20260712000013_add_viewer_role.sql` was also pushed and is now identical to the viewer role fix applied earlier.
