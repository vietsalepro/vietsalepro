...
BEGIN
  ...
  IF v_disposal_status IS NULL THEN
    RAISE EXCEPTION 'Phiếu xuất hủy không tồn tại (%)', p_disposal_id;
  END IF;

  IF v_disposal_status = 'COMPLETED' THEN
    RAISE EXCEPTION 'Phiếu xuất hủy đã hoàn thành trước đó';
  END IF;
  ...
  LOOP
    ...
    IF v_product_quantity IS NULL THEN
      RAISE EXCEPTION 'Sản phẩm % không tồn tại trong kho', v_item.product_id;
    END IF;

    IF v_product_quantity < v_item.quantity THEN
      RAISE EXCEPTION 'Sản phẩm % không đủ tồn kho (còn %, cần hủy %)',
    ...
    IF v_item.lot_id IS NOT NULL THEN
      ...
      IF v_lot_quantity IS NULL THEN
        RAISE EXCEPTION 'Lô % không tồn tại cho sản phẩm %',
          v_item.lot_id, v_item.product_id;
      END IF;

      IF v_lot_quantity < v_item.quantity THEN
        RAISE EXCEPTION 'Lô % không đủ số lượng (còn %, cần hủy %)',
