...
const PRIORITY_SLA_HOURS: Record<SupportTicketPriority, number> = {
  low: 72,
  medium: 24,
  high: 8,
  urgent: 2,
...
// ponytail: SLA target is computed from client time. For stricter SLA enforcement,
// move this to an RPC/trigger using database now() so server clock is the source of truth.
export function computeSlaTargetAt(priority?: SupportTicketPriority): string | undefined {
...
export async function createSupportTicket(input: CreateSupportTicketInput): Promise<SupportTicket> {
  ...
  const insert: Record<string, any> = {
    ...
    priority,
    sla_target_at: computeSlaTargetAt(priority),
  };
...
export async function updateSupportTicket(
  ...
  if (input.priority !== undefined) {
    ...
    // ponytail: recompute SLA target whenever priority changes.
    update.sla_target_at = computeSlaTargetAt(input.priority);
  }
  ...
  if (input.slaTargetAt !== undefined) update.sla_target_at = input.slaTargetAt;
