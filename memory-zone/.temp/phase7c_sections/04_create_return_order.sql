-- =====================================================================
-- 4. Tráº£ hÃ ng â€” Delivery Note (reverse)
-- =====================================================================
...
BEGIN
  ...
  IF jsonb_typeof(p_items) != 'array' OR jsonb_array_length(p_items) = 0 THEN
    RAISE EXCEPTION 'Phiáº¿u tráº£ pháº£i cÃ³ Ã­t nháº¥t 1 sáº£n pháº©m';
  END IF;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    PERFORM insert_stock_ledger_entry(
      ...
      v_valuation_rate,
      0::NUMERIC,
      COALESCE(v_item->>'reason', 'Tráº£ hÃ ng'),
      FALSE
    );
  END LOOP;
  ...
  IF v_customer_id_clean IS NOT NULL THEN
    ...
    IF v_points_to_deduct > 0 THEN
      ...
      VALUES (
        ...
        'return',
        -v_points_to_deduct,
        'Trá»« Ä‘iá»ƒm do tráº£ hÃ ng tá»« Ä‘Æ¡n ' || p_original_order_id,
        p_original_order_id
      );
    END IF;
  END IF;
