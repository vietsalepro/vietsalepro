## Why

P13.4 — Bulk operations + maintenance scheduler: maintenance_windows + calendar UI.

## What Changes

- Implement P13 4 Bulk Maintenance per KE_HOACH_ADMIN_DASHBOARD_SUB_PHASE.md.
- See design.md for technical decisions and tasks.md for implementation steps.

## Scope / Non-Goals

**In scope:**
- P13 4 Bulk Maintenance

**Out of scope:**
- Other admin dashboard phases not listed here.

## Capabilities

### New Capabilities
- `p13-4-bulk-maintenance`: P13.4 — Bulk operations + maintenance scheduler: maintenance_windows + calendar UI.

### Modified Capabilities
- None outside this sub-phase.

## Impact

- Affected files: see design.md.
- Verification: see Acceptance Criteria in tasks.md.

## Rollback

Restore affected files and database changes from the pre-phase backup. Detailed steps in rollback.md.
