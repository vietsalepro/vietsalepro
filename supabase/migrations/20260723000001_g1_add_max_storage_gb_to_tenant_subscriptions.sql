-- G1: Extend the canonical subscription-update contract to accept p_max_storage_gb.
-- ponytail: schema evolution + canonical function evolution only; no new RPC.

-- ============================================================
-- 1. Schema extension: storage limit on tenant_subscriptions
-- ============================================================
ALTER TABLE public.tenant_subscriptions
  ADD COLUMN IF NOT EXISTS max_storage_gb integer NOT NULL DEFAULT 1;

-- ============================================================
-- 2. Canonical function evolution
-- ============================================================
-- Drop the prior 7-parameter overload so the canonical contract remains a single function.
DROP FUNCTION IF EXISTS public.update_tenant_subscription(UUID, TEXT, INTEGER, INTEGER, INTEGER, TEXT, TIMESTAMPTZ);
...
BEGIN
  ...
  v_new_max_storage := COALESCE(p_max_storage_gb, CASE WHEN p_plan IS NOT NULL THEN COALESCE((v_limits->>'max_storage_gb')::INTEGER, v_sub.max_storage_gb) ELSE v_sub.max_storage_gb END);

  IF v_new_max_users <= 0 OR v_new_max_products <= 0 OR v_new_max_orders <= 0 OR v_new_max_storage <= 0 THEN
  ...
  SET plan = v_new_plan,
      ...
      max_storage_gb = v_new_max_storage,
