...
> Capability: voucher-form-component-system
...
Provide a shared component system for voucher-style forms (Nhập hàng, Xuất hủy, Kiểm kê, Đổi hàng NCC) so that all 4 forms share consistent layout, controls, search, table, and empty-state patterns while preserving business logic and public APIs.
...
The system SHALL replace old UI shells in `pages/InventoryCount.tsx` with voucher-form components.

#### Scenario: InventoryCount form refactor
- **GIVEN** `pages/InventoryCount.tsx` form view
- **WHEN** refactored with `VoucherFormLayout`, `VoucherSearch`, `VoucherProductDropdown`, `VoucherTable`, `VoucherTableRow`, `VoucherSection`, `VoucherField`, `VoucherInput`, `VoucherButton`, and `VoucherEmpty`
- **THEN** product search, table, Excel import, scan, diff, save draft, and complete remain functional

### Requirement: Rollout on SupplierExchanges
The system SHALL keep the wizard flow and replace old UI shells with voucher-form components.

#### Scenario: SupplierExchanges create form refactor
- **GIVEN** `pages/SupplierExchanges.tsx` create flow
- **WHEN** refactored with `VoucherFormLayout`, `VoucherSearch`, `VoucherProductDropdown`, `VoucherSection`, `VoucherField`, `VoucherInput`, `VoucherButton`, and wizard item cards
- **THEN** product search, lot selection, receipt selection, item cards, and complete remain functional

### Requirement: Dead code cleanup
The system SHALL remove obsolete components and CSS after the 4 pages are fully migrated.

#### Scenario: Cleanup candidates
- **GIVEN** the 4 pages no longer import old UI shells
- **WHEN** the old components are removed
- **THEN** `npm run lint` and `npm run build` pass and the old files are deleted
