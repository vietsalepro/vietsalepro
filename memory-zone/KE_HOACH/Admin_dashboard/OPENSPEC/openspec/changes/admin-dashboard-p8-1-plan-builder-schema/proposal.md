## Why

P8.1 — Plan builder schema: plans table + migrate Free/VIP (YAGNI).

## What Changes

- Implement P8 1 Plan Builder Schema per KE_HOACH_ADMIN_DASHBOARD_SUB_PHASE.md.
- See design.md for technical decisions and tasks.md for implementation steps.

## Scope / Non-Goals

**In scope:**
- P8 1 Plan Builder Schema

**Out of scope:**
- Other admin dashboard phases not listed here.

## Capabilities

### New Capabilities
- `p8-1-plan-builder-schema`: P8.1 — Plan builder schema: plans table + migrate Free/VIP (YAGNI).

### Modified Capabilities
- None outside this sub-phase.

## Impact

- Affected files: see design.md.
- Verification: see Acceptance Criteria in tasks.md.

## Rollback

Restore affected files and database changes from the pre-phase backup. Detailed steps in rollback.md.
