## Why

P15.1 — API platform: tenant_api_keys + create/revoke + auth middleware + versioning (YAGNI).

## What Changes

- Implement P15 1 Api Keys per KE_HOACH_ADMIN_DASHBOARD_SUB_PHASE.md.
- See design.md for technical decisions and tasks.md for implementation steps.

## Scope / Non-Goals

**In scope:**
- P15 1 Api Keys

**Out of scope:**
- Other admin dashboard phases not listed here.

## Capabilities

### New Capabilities
- `p15-1-api-keys`: P15.1 — API platform: tenant_api_keys + create/revoke + auth middleware + versioning (YAGNI).

### Modified Capabilities
- None outside this sub-phase.

## Impact

- Affected files: see design.md.
- Verification: see Acceptance Criteria in tasks.md.

## Rollback

Restore affected files and database changes from the pre-phase backup. Detailed steps in rollback.md.
